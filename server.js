...
app.post('/api/order', async (req, res) => {
  try {
    const { mode = 'paper', symbol, notional, qty, side = 'buy', type = 'market', time_in_force = 'day' } = req.body;
    const a = alpaca(mode === 'live' ? 'live' : 'paper');
    if (!a.key) return res.status(400).json({ error: `Alpaca ${mode} keys not configured on server.` });
    if (!symbol) return res.status(400).json({ error: 'Missing symbol.' });

    const order = { symbol: symbol.toUpperCase(), side, type, time_in_force };
    ...
    res.json({ success: true, order: { id: d.id, status: d.status, symbol: d.symbol, side: d.side, notional: d.notional, qty: d.qty } });
  } catch (e) {
  ...
  try {
    ...
    for (const feed of RSS_FEEDS) {
      try {
        ...
        // crude but dependency-free item extraction
        const items = xml.split('<item>').slice(1, 5);
        const source = feed.includes('bbc') ? 'BBC' : feed.includes('marketwatch') ? 'MARKETWATCH' : 'NEWS';
        items.forEach(it => {
